...
public class AnimLayer extends FrameLayout {
	...
	public void createMoveAnim(final Card from,final Card to,int fromX,int toX,int fromY,int toY){
		...
		LayoutParams lp = new LayoutParams(GameConfig.CARD_WIDTH, GameConfig.CARD_WIDTH);
		lp.leftMargin = fromX*GameConfig.CARD_WIDTH;
		lp.topMargin = fromY*GameConfig.CARD_WIDTH;
		c.setLayoutParams(lp);
		
		if (to.getNum()<=0) {
			to.getLabel().setVisibility(View.INVISIBLE);
		}
		TranslateAnimation ta = new TranslateAnimation(0, GameConfig.CARD_WIDTH*(toX-fromX), 0, GameConfig.CARD_WIDTH*(toY-fromY));
		ta.setDuration(100);
